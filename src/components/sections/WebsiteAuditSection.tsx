...
const scanStatuses = [
  "Crawling pages…", "Analyzing SEO tags…", "Checking page speed…",
  "Auditing mobile responsiveness…", "Scanning accessibility…",
  "Evaluating best practices…", "Generating report…",
];
...
function CircularGauge({ score, label, delay = 0 }: { score: number; label: string; color?: string; delay?: number }) {
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <span className="type-label text-muted-foreground font-mono">{label}</span>
    </div>
  );
}
...
export default function WebsiteAuditSection() {
  ...
  const startScan = useCallback(() => {
    ...
    if (!parsed.success) { setUrlError("Please enter a valid website URL"); return; }
    setUrlError("");
    setStep("scanning");
    setScanProgress(0);
    setScanStatus(0);
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => {
      ...
      if (elapsed >= total) { clearInterval(timer); setTimeout(() => setStep("results"), 500); }
    }, interval);
  ...
  return (
    <section className="py-28 md:py-36 relative overflow-hidden" ref={sectionRef}>
      <div className="container mx-auto px-4 relative z-10">
        ...
        >
          <span className="type-label text-primary font-mono">Free Website Audit</span>
          <h2 className="font-display type-h2 text-foreground mt-3 mb-4">
            Check Your Website's Health <span className="text-primary">in 30 Seconds</span>
          </h2>
          ...
          <AnimatePresence mode="wait">
            {step === "input" && (
              <motion.div key="input" initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.97 }}
                className="rounded-2xl card-surface ring-1 ring-primary/20 p-8 md:p-10"
              >
                ...
                <div className="flex flex-col sm:flex-row gap-3">
                  <div className="flex-1">
                    <Input placeholder="https://yourwebsite.com" value={url} onChange={e => { setUrl(e.target.value); setUrlError(""); }} className="h-12 text-base rounded-xl" />
                ...
                <div className="grid md:grid-cols-2 gap-8">
                  <div className="flex flex-col items-center justify-center">
                    <div className="relative w-40 h-40 mb-6">
                      ...
                      <div className="absolute inset-4 rounded-full" style={{ border: "1px solid rgba(255,255,255,0.03)" }} />
                    ...
                    <div className="w-full max-w-xs h-2 rounded-full bg-border overflow-hidden">
                      <div className="h-full rounded-full bg-primary transition-all duration-100" style={{ width: `${scanProgress}%` }} />
                    </div>
                  ...
                  <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 1.5, duration: 0.6 }}>
                    ...
                    {!leadSubmitted ? (
                      <div className="space-y-3">
                        <Input placeholder="Your Name" value={leadForm.name} onChange={e => setLeadForm(p => ({ ...p, name: e.target.value }))} className="h-11 rounded-xl" maxLength={100} />
                        <Input placeholder="Email Address" type="email" value={leadForm.email} onChange={e => setLeadForm(p => ({ ...p, email: e.target.value }))} className="h-11 rounded-xl" maxLength={255} />
                        <Input placeholder="WhatsApp Number" value={leadForm.whatsapp} onChange={e => setLeadForm(p => ({ ...p, whatsapp: e.target.value }))} className="h-11 rounded-xl" maxLength={20} />
                        <Button onClick={submitLead} disabled={submitting || !leadForm.name || !leadForm.email} className="w-full rounded-xl font-display font-bold h-11">
            ...
            {step === "results" && (
              ...
              >
                <div className="text-center mb-8">
                  <p className="type-label text-muted-foreground font-mono mb-3">WEBSITE HEALTH SCORE</p>
                  <CircularGauge score={scores.current.overall} label="Overall" />
                  <p className="text-sm text-muted-foreground mt-3 font-display">Your website needs improvement in several areas</p>
                </div>

                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8 pb-8 border-b border-border">
                  <CircularGauge score={scores.current.seo} label="SEO" delay={200} />
                  <CircularGauge score={scores.current.performance} label="Performance" delay={400} />
                  <CircularGauge score={scores.current.accessibility} label="Accessibility" delay={600} />
                  <CircularGauge score={scores.current.bestPractices} label="Best Practices" delay={800} />
                </div>
                ...
                <div className="space-y-2 mb-8">
                  {mockIssues.map((issue, i) => (
                    <motion.div
                      ...
                      className="flex items-start gap-3 rounded-xl card-surface p-3"
                    >
                      <AlertTriangle className={`w-4 h-4 mt-0.5 shrink-0 ${
                        issue.severity === "high" ? "text-red-400" : issue.severity === "medium" ? "text-amber-400" : "text-blue-400"
                      }`} />
                      <span className="text-sm text-foreground/80">{issue.label}</span>
                      <span className={`ml-auto type-label font-mono px-2 py-0.5 rounded-full ${
                        issue.severity === "high" ? "bg-red-500/10 text-red-400" : issue.severity === "medium" ? "bg-amber-500/10 text-amber-400" : "bg-blue-500/10 text-blue-400"
                      }`}>{issue.severity}</span>
                    </motion.div>
                  ))}
                </div>
                ...
                <div className="text-center">
                  ...
                  <p className="text-xs text-muted-foreground mt-3">Our experts will create a custom strategy to fix these issues</p>
                </div>

                {!leadSubmitted && (
                  <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="mt-8 pt-8 border-t border-border">
                    <p className="font-display font-bold text-foreground text-center mb-4">📩 Email this report to yourself</p>
                    <div className="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
                      <Input placeholder="Your Name" value={leadForm.name} onChange={e => setLeadForm(p => ({ ...p, name: e.target.value }))} className="h-11 rounded-xl" maxLength={100} />
                      <Input placeholder="Email" type="email" value={leadForm.email} onChange={e => setLeadForm(p => ({ ...p, email: e.target.value }))} className="h-11 rounded-xl" maxLength={255} />
                      <Button onClick={submitLead} disabled={submitting || !leadForm.name || !leadForm.email} className="rounded-xl font-display font-bold h-11 shrink-0">
